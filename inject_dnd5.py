...
# Replace Grid FIRST
start_marker = '<div className="widget-box widget-bank" onClick={() => setActiveWidget(\'bank\')}>'
end_marker = '<span className="widget-title">Giderler (Bu Ay)</span>\n            </div>\n            <p className="widget-value">{formatMoney(currentMonthExpense)}</p>\n          </div>'
...
if start_index != -1 and end_index != -1:
    ...
    # The replacement is just the map function
    code = code[:start_index] + "{widgetOrder.map((id, index) => renderWidget(id, index))}" + code[end_index:]
else:
    # Try alternate end marker (spacing differences)
    end_marker = '<span className="widget-title">Giderler (Bu Ay)</span>\n          </div>\n          <p className="widget-value">{formatMoney(currentMonthExpense)}</p>\n        </div>'
    end_index = code.find(end_marker)
    if start_index != -1 and end_index != -1:
        end_index += len(end_marker)
