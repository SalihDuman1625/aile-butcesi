...
if 'renderWidget' not in code:
    code = code.replace(
        "const renderWidgetModal = () => {",
        render_fn_code + "\n  const renderWidgetModal = () => {"
    )

# Replace the specific block of 8 widgets using precise string replacement
target_block = """        <div className="dashboard-grid">
...
new_block = """        <div className="dashboard-grid">
...
if target_block in code:
    ...
    print("TARGET BLOCK NOT FOUND! Using regex but VERY specific one.")
    # Safe regex that only matches up to Giderler </div>
    code = re.sub(r'<div className="widget-box widget-bank".*?<span className="widget-title">Giderler \(Bu Ay\)<\/span>.*?<\/div>\s*<\/div>', '{widgetOrder.map((id, index) => renderWidget(id, index))}', code, flags=re.DOTALL)
